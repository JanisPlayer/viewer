...
export default {
	...
	methods: {
		...
		updateZoomAndShift(stableX, stableY, newZoomRatio) {
			...
			const scrollX = stableX - this.$el.getBoundingClientRect().x - (this.width * this.zoomRatio / 2)
			const scrollY = stableY - this.$el.getBoundingClientRect().y - (this.height * this.zoomRatio / 2)
